		...
		for building in GameInfo.Buildings() do
			...
			if thisBuildingClass.MaxGlobalInstances <= 0 and thisBuildingClass.MaxPlayerInstances ~= 1 and thisBuildingClass.MaxTeamInstances <= 0 then
				...
				if pCity:GetNumSpecialistsAllowedByBuilding(buildingID) <= 0 then
					...
					-- MOD - TOKATA : City Halls and City-Level buildings shall not display here
						if buildingID == GameInfo.Buildings.BUILDING_CITY_HALL_LV1.ID or buildingID == GameInfo.Buildings.BUILDING_SATRAPS_COURT.ID or buildingID == GameInfo.Buildings.BUILDING_CITY_HALL_LV2.ID or buildingID == GameInfo.Buildings.BUILDING_CITY_HALL_LV3.ID or buildingID == GameInfo.Buildings.BUILDING_CITY_HALL_LV4.ID or buildingID == GameInfo.Buildings.BUILDING_CITY_HALL_LV5.ID or buildingID == GameInfo.Buildings.BUILDING_PUPPET_GOVERNEMENT.ID or buildingID == GameInfo.Buildings.BUILDING_PUPPET_GOVERNEMENT_FULL.ID then
							bCityHall = building		
							
						elseif GameInfo.Buildings[buildingID].GreatWorkCount < iCityLevel then
							iCityLevel = GameInfo.Buildings[buildingID].GreatWorkCount
							bCityLevel = building
							
						elseif GameInfo.Buildings[buildingID].GreatWorkCount == 0 then
...
function OnYes( )
	...
	if Players[Game.GetActivePlayer()]:IsTurnActive() then
		...
		if g_iBuildingToSell == GameInfo.Buildings["BUILDING_CITY_HALL_LV1"].ID or g_iBuildingToSell == GameInfo.Buildings["BUILDING_CITY_HALL_LV2"].ID or g_iBuildingToSell == GameInfo.Buildings["BUILDING_CITY_HALL_LV3"].ID or g_iBuildingToSell == GameInfo.Buildings["BUILDING_CITY_HALL_LV4"].ID or g_iBuildingToSell == GameInfo.Buildings["BUILDING_CITY_HALL_LV5"].ID then
			...
			pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_BARRACKS"],0)
			pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_ARMORY"],0)
			pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_ARSENAL"],0)
			pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_MILITARY_BASE"],0)
			
			pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_REPRESENTATION_CULTURE"],0)	  
...
--		if g_iBuildingToSell == GameInfo.Buildings["BUILDING_CONSTABLE"].ID then
--		   pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_CONSTABLE_I"],0)
--		end 
--		
--		if g_iBuildingToSell == GameInfo.Buildings["BUILDING_POLICE_STATION"].ID then
--		   pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_POLICE_STATION_I"],0)
--		   pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_POLICE_STATION_II"],0)
--		end 
